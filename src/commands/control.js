...
const go = (end, speed) =>
    commander.send(`go ${end.x} ${end.y} ${end.z} ${speed}`);

const curve = (start, end, speed) =>
    commander.send(
        `curve ${start.x} ${start.y} ${start.z} ${end.x} ${end.y} ${end.z} ${speed}`
    );

const jump = (start, speed, yaw, mid1, mid2) =>
    commander.send(`jump ${start.x} ${start.y} ${start.z} ${speed} ${yaw}
${mid1} ${mid2}`);

const reboot = () => commander.send("reboot");
